...
def get_thumbnail(asset_id):
    """Obtém imagem do asset para o webhook."""
...
def notify_roblox(status, asset_id="N/A", player_id="0"):
    """Feedback via Messaging Service."""
    ...
    try: requests.post(url, headers={"x-api-key": API_KEY, "Content-Type": "application/json"}, json=data, timeout=10)
    except: pass
...
def main():
    ...
    # 1. Download Autenticado (Evita 401)
    file_path = "item.rbxm"
    try:
    ...
    sys.exit(0) # Força saída com código 0 para o GitHub Actions
